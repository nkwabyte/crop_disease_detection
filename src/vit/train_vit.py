#!/usr/bin/env python3
"""
train_vit.py — ViTDet crop-disease detector (Vision Transformer backbone + Faster R-CNN head).

A Vision Transformer detector for the two-stage pipeline and the mobile app. The
backbone is a torchvision ViT-B/16 (ImageNet-pretrained) whose patch tokens are
reshaped into a 2-D feature map and fed to the *same* Faster R-CNN detection head
(RPN + RoI) used by the ResNet pipeline. This gives a clean apples-to-apples
"CNN backbone vs Transformer backbone" comparison for the paper, while producing
an ExecuTorch (.pte) artifact the Android/Kotlin app can consume like the others.

Design notes
------------
  • ViT positional embeddings are resolution-specific, so the input is forced to a
    fixed IMG_SIZE×IMG_SIZE square via GeneralizedRCNNTransform(fixed_size=...).
    ImageNet pretrained pos-embeddings (14×14 grid @224) are bicubic-interpolated
    to the 40×40 grid @640 with torchvision's `interpolate_embeddings`.
  • Single-scale feature map (stride 16). A 1×1 projection maps the 768-dim tokens
    to 256 channels to match the detection head's expected width.
...
than imported from src/fasterrcnn/, so the two packages stay fully decoupled.

Usage
-----
  python -m src.vit.train_vit                    # full pipeline (steps 1–4)
  python -m src.vit.train_vit --dry-run          # 2-epoch timing estimate
  python -m src.vit.train_vit --skip-negatives   # skip hard-negative download
  ...
  python -m src.vit.train_vit --batch-size 4      # override batch size (e.g. on CUDA)
  DRY_RUN=1 python -m src.vit.train_vit           # dry-run via env var
"""
...
from src.vit.config import (
    ...
    PATCH_SIZE, VIT_HIDDEN_DIM, FPN_OUT_CHANNELS,
...
# ══════════════════════════════════════════════════════════════════════════════
# Model — ViT-B/16 backbone + Faster R-CNN head
# ══════════════════════════════════════════════════════════════════════════════

def _build_vit(img_size: int, pretrained: bool = True) -> VisionTransformer:
    """Return a ViT-B/16 whose positional embeddings are sized for img_size×img_size.

    Loads ImageNet weights at 224 and bicubic-interpolates the pos-embeddings to the
    target token grid (img_size/16)². Falls back to random init if weights are offline.
    """
    weights = ViT_B_16_Weights.IMAGENET1K_V1 if pretrained else None
    try:
        src = vit_b_16(weights=weights)
    except Exception as exc:
        print(f"  [WARN]  Could not load pretrained ViT weights ({exc}); using random init.")
        src = vit_b_16(weights=None)

    if img_size == src.image_size:
        return src

    model = VisionTransformer(
        image_size=img_size, patch_size=PATCH_SIZE,
        num_layers=12, num_heads=12, hidden_dim=VIT_HIDDEN_DIM, mlp_dim=3072,
    )
    new_state = interpolate_embeddings(img_size, PATCH_SIZE, src.state_dict())
    model.load_state_dict(new_state)
    return model


class ViTDetBackbone(nn.Module):
    """ViT-B/16 encoder → 2-D feature map (stride 16) → 1×1 projection to out_channels."""

    def __init__(self, img_size: int = IMG_SIZE, out_channels: int = FPN_OUT_CHANNELS,
                 pretrained: bool = True):
        super().__init__()
        self.vit         = _build_vit(img_size, pretrained)
        self.hidden_dim  = self.vit.hidden_dim
        self.patch_size  = self.vit.patch_size
        self.grid        = img_size // self.patch_size
        self.out_channels = out_channels
        self.proj = nn.Sequential(
            nn.Conv2d(self.hidden_dim, out_channels, kernel_size=1),
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        n = x.shape[0]
        x = self.vit._process_input(x)                       # [B, N, hidden]
        cls = self.vit.class_token.expand(n, -1, -1)
        x = torch.cat([cls, x], dim=1)                       # [B, N+1, hidden]
        x = self.vit.encoder(x)                              # + pos-embed, blocks, LN
        x = x[:, 1:, :]                                       # drop class token
        x = x.permute(0, 2, 1).reshape(n, self.hidden_dim, self.grid, self.grid)
        x = self.proj(x)                                     # [B, out, grid, grid]
        out: Dict[str, torch.Tensor] = {"0": x}
        return out


def build_model(num_classes: int = NUM_CLASSES, img_size: int = IMG_SIZE,
                pretrained: bool = True) -> FasterRCNN:
    backbone = ViTDetBackbone(img_size, FPN_OUT_CHANNELS, pretrained)
    ...
    # Force an exact square resize so the ViT always receives img_size×img_size
    # (its positional embeddings are fixed for that grid).
    model.transform = GeneralizedRCNNTransform(
        img_size, img_size, mean, std, fixed_size=(img_size, img_size),
    )
    return model


def set_backbone_grad(model: nn.Module, requires_grad: bool) -> None:
    """Toggle grads on the ViT encoder (the projection head stays trainable)."""
...
def log_startup(device, n_train, n_val, epochs, batch, dry_run):
    sep = "─" * 66
    print(f"\n{sep}")
    print(f"  Model        : ViTDet (ViT-B/16 backbone + Faster R-CNN head)")
    print(f"  Device       : {device}")
    print(f"  Batch size   : {batch}  (accum {ACCUM_STEPS} → effective {batch * ACCUM_STEPS})")
    print(f"  Image size   : {IMG_SIZE}×{IMG_SIZE}  (fixed square; {IMG_SIZE // PATCH_SIZE}² tokens)")
...
def save_final_eval(result: dict, model: nn.Module, split: str = "test") -> None:
    ...
    payload = {
        "model_name": "vitdet_vit_b16",
        "architecture": "ViT-B/16 backbone + Faster R-CNN head",
        "split": split,
    ...
    try:
        ...
        with open(BENCHMARK_DIR / "vitdet_vit_b16.json", "w") as f:
            json.dump(payload, f, indent=2)
    except Exception as exc:
...
def generate_figures(pre_only: bool = False) -> None:
    ...
    try:
        ...
        fig.suptitle("ViTDet — Dataset Overview", fontweight="bold")
        fig.tight_layout()
    ...
    # ── fig_02 — ViTDet architecture schematic ───────────────────────────────
    try:
        fig, ax = plt.subplots(figsize=(12, 3.6))
        ax.set_xlim(0, 12); ax.set_ylim(0, 3); ax.axis("off")
        stages = [
            ("Input\n640×640×3", "#AAB7C4"),
            (f"Patchify 16×16\n{IMG_SIZE // PATCH_SIZE}×{IMG_SIZE // PATCH_SIZE} tokens", "#5B8FF9"),
            ("+ Pos-embed\n(interp. 224→640)", "#5B8FF9"),
            ("12× Transformer\nEncoder blocks", "#3E6FE0"),
            ("Reshape tokens\n→ 40×40×768", "#61DDAA"),
            ("1×1 proj → 256\nRPN + RoI head", "#F6BD16"),
            ("Boxes + 23\nclasses", "#E8684A"),
        ]
        w = 1.5; gap = 0.25; x = 0.15
        for i, (txt, col) in enumerate(stages):
            box = FancyBboxPatch((x, 0.9), w, 1.2, boxstyle="round,pad=0.04",
                                 fc=col, ec="black", lw=0.8, alpha=0.9)
            ax.add_patch(box)
            ax.text(x + w / 2, 1.5, txt, ha="center", va="center", fontsize=8.5, wrap=True)
            if i < len(stages) - 1:
                ax.add_patch(FancyArrowPatch((x + w, 1.5), (x + w + gap, 1.5),
                             arrowstyle="-|>", mutation_scale=12, lw=1.2, color="black"))
            x += w + gap
        ax.set_title("ViTDet — ViT-B/16 backbone feeding a Faster R-CNN detection head",
                     fontweight="bold", fontsize=11)
        fig.tight_layout()
        fig.savefig(OUTPUT_DIR / "fig_02_vit_architecture.png", bbox_inches="tight")
        plt.close(fig)
    except Exception as exc:
    ...
    try:
        ...
        ax.set_title("ViTDet — AdamW LR schedule (linear warmup → cosine decay)", fontweight="bold")
        ax.legend()
        fig.tight_layout()
    ...
    if METRICS_FILE.exists():
        try:
            ...
            fig.suptitle("ViTDet — Training Metrics", fontweight="bold")
            fig.tight_layout()
    ...
    if FINAL_EVAL_FILE.exists():
        try:
            ...
            ax.set_title(f"ViTDet — Per-class AP@0.5  (mAP={fe['map50']:.3f})", fontweight="bold")
...
class _BackboneWrapper(nn.Module):
    """Feature-extractor wrapper: normalized image tensor → single-scale feature map.

    Static shapes throughout (pure ViT ops), so this exports cleanly to ExecuTorch.
    """
...
def export_model(model: nn.Module) -> None:
    ...
    try:
        ...
        ptl_path  = MODELS_DIR / "crop_disease_vit.ptl"
        ...
        (MODELS_DIR / "crop_disease_vit.ptl").unlink(missing_ok=True)
        try:
            scripted = torch.jit.script(model)
            pt_path  = MODELS_DIR / "crop_disease_vit_jit.pt"
    ...
    try:
        ...
        onnx_path = MODELS_DIR / "crop_disease_vit.onnx"
        torch.onnx.export(
    ...
    try:
        ...
        # 3a. Backbone-only — static-shape ViT, exports reliably.
        ...
        pte_bb = MODELS_DIR / "crop_disease_vit_backbone.pte"
        ...
        try:
            ...
            pte_full = MODELS_DIR / "crop_disease_vit.pte"
    ...
    import yaml
    metadata = {
        "model_name": "crop_disease_vitdet",
        "architecture": "ViT-B/16 backbone + Faster R-CNN head (ViTDet-style)",
        ...
        "android_integration": {
            "runtime": "ExecuTorch",
            "backend": "XNNPACK",
            "pte_file": "crop_disease_vit.pte",
            "backbone_pte_file": "crop_disease_vit_backbone.pte",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ViTDet crop-disease detector")
    ...
    parser.add_argument("--no-pretrained", action="store_true", help="random-init ViT (no ImageNet)")
    ...
    if args.figures_only:
        print("Regenerating ViTDet figures …")
